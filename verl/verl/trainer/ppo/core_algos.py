...
def compute_grpo_ema_outcome_advantage(
    ...
    """Compute advantage for EMA-GRPO using cross-epoch EMA statistics.

    Replaces per-batch mean/std with per-prompt exponential moving averages
    maintained in ``memory_bank``, reducing the high variance of single-batch
    advantage estimates (especially for 0/1 rewards with small rollout groups).

    During warm-up (prompt seen fewer than ``memory_bank.warmup_steps`` times),
    falls back to standard per-batch GRPO normalization.

    Args:
        token_level_rewards: shape (bs, response_length)
        response_mask: shape (bs, response_length)
        index: per-sample uid used for intra-batch grouping (same as GRPO)
        prompt_index: stable dataset index for each sample (cross-epoch identity)
        memory_bank: PromptMemoryBank instance (updated in-place after this call)
        epsilon: small value to avoid division by zero
        norm_adv_by_std_in_grpo: whether to scale by std
        config: algorithm configuration (unused, kept for registry compatibility)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            id2prompt_key[uid] = prompt_index[i]

        id2mean: dict = {}
        id2soft_weight: dict = {}
        use_soft = memory_bank.mastery_soft_alpha > 0.0

        for uid, uid_scores in id2scores.items():
            scores_tensor = torch.stack(uid_scores)
            if len(scores_tensor) == 1:
                batch_mean_val = 0.0
                batch_std_val = 0.0
                batch_mean_t = torch.tensor(0.0, dtype=scores.dtype)
            else:
                batch_mean_t = torch.mean(scores_tensor)
                batch_std_val = torch.std(scores_tensor).item()
                batch_mean_val = batch_mean_t.item()

            dataset_idx = id2prompt_key[uid]

            # Update bank first to obtain μ_t (the EMA mean incorporating the current batch).
            # μ_t is then used both for the mastery filter and as the advantage baseline,
            # so the filter reflects the model's sustained accuracy rather than a single batch.
            memory_bank.update(dataset_idx, batch_mean_val, batch_std_val)
            ema_mean_t, _, _ = memory_bank.query(dataset_idx)

            # Advantage = r - μ_t (no std division, following Dr. GRPO).
            # Mastery filter: if μ_t > τ = (n-1)/n, the model has consistently mastered
            # this prompt; fall back to batch mean → advantage = 0, zero gradient.
            if use_soft:
                id2mean[uid] = torch.tensor(ema_mean_t, dtype=scores.dtype)
                id2soft_weight[uid] = memory_bank.soft_mastery_weight(dataset_idx)
            elif memory_bank.is_mastered_batch(ema_mean_t):
                # μ_t > τ: sustained mastery confirmed — zero gradient.
                id2mean[uid] = batch_mean_t
            else:
                id2mean[uid] = torch.tensor(ema_mean_t, dtype=scores.dtype)
        ...
        for i in range(bsz):
            ...
            if uid in id2soft_weight:
                scores[i] = scores[i] * id2soft_weight[uid]
